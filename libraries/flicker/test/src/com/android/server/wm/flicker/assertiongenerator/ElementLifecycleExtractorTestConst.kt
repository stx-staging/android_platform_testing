...
class ElementLifecycleExtractorTestConst {
    companion object {
        ...
        val layer_id5_sameComponentMatcher = createTestLayerWithEmptyRegion("StatusBar", 1, 5, 0)

        val layer_id6_t1 = null
        val layer_id6_t2 = createTestLayerWithEmptyRegion("openPackage/openApp", 6, 6, 0)
        val layer_id6_t3 = createTestLayer("openPackage/openApp", 8, 6, 0)

        val mapOfFlattenedLayers: Map<Int, Array<Layer>> =
            mapOf(
        ...
        val mapOfFlattenedLayersAssertionProducer: Map<Int, Array<Layer>> =
            mapOf(
        ...
        val mapOfFlattenedLayersAllVisibilityAssertions: Map<Int, Array<Layer>> =
            mapOf(
                1 to arrayOf(),
        ...
        val mapOfFlattenedLayers_SameComponentMatcher: Map<Int, Array<Layer>> =
            mapOf(
        ...
        val expectedElementLifecycle_id1 =
            LayersElementLifecycle(mutableListOf(layer_id1_t1, layer_id1_t2, null))

        val expectedElementLifecycle_id2 =
            LayersElementLifecycle(mutableListOf(layer_id2_t1, layer_id2_t2, layer_id2_t3))

        val expectedElementLifecycle_id3 =
            LayersElementLifecycle(mutableListOf(null, layer_id3_t2, null))

        val expectedElementLifecycle_id4 =
            LayersElementLifecycle(mutableListOf(layer_id4_t1, layer_id4_t2, layer_id4_t3))

        val expectedElementLifecycle_id6 =
            LayersElementLifecycle(mutableListOf(layer_id6_t1, layer_id6_t2, layer_id6_t3))

        val expectedElementLifecycle_id5_sameComponentMatcher =
        ...
        val expectedComponentMatcherLifecycle_Navbar =
            LayersComponentLifecycle(mutableMapOf(1 to expectedElementLifecycle_id1))

        val expectedComponentMatcherLifecycle_Statusbar =
            LayersComponentLifecycle(mutableMapOf(2 to expectedElementLifecycle_id2))

        val expectedComponentMatcherLifecycle_Launcher =
            LayersComponentLifecycle(mutableMapOf(4 to expectedElementLifecycle_id4))

        val expectedComponentMatcherLifecycle_OpenApp =
            LayersComponentLifecycle(mutableMapOf(6 to expectedElementLifecycle_id6))
        ...
        val expectedElementLifecycles_SameComponentMatcher =
            mapOf(
        ...
        val expectedFailElementLifecycle1_AllVisibilityAssertions_id2 =
            LayersElementLifecycle(mutableListOf(layer_id2_t1, layer_id2_t2))

        val mapOfFlattenedLayersAllVisibilityAssertions_fail1: Map<Int, Array<Layer>> =
            mapOf(
                1 to arrayOf(layer_id2_t1),
                2 to arrayOf(layer_id2_t2),
            )

        val expectedFailElementLifecycle2_AllVisibilityAssertions_id2 =
            LayersElementLifecycle(mutableListOf(null, layer_id2_t1))

        // Passed                : notContains(StatusBar)
        // Assertion never failed: isVisible(StatusBar)
        // Untested              : isInvisible(StatusBar)
        // Trace                 : 1/3:[notContains(StatusBar)]	Entry: 1/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=1))
        // Trace                 : 1/3:[notContains(StatusBar)]	Entry: 2/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=2))
        // Trace                 : 2/3:[isVisible(StatusBar)]	Entry: 2/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=2))
        // Trace                 : 2/3:[isVisible(StatusBar)]	Entry: 3/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=3))
        val mapOfFlattenedLayersAllVisibilityAssertions_fail2: Map<Int, Array<Layer>> =
            mapOf(1 to arrayOf(), 2 to arrayOf(layer_id2_t1), 3 to arrayOf(layer_id2_t1))

        val expectedFailElementLifecycle3_AllVisibilityAssertions_id2 =
            LayersElementLifecycle(mutableListOf(null))
        ...
        // Trace                 : 1/3:[notContains(StatusBar)]	Entry: 2/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=2))
        // Trace                 : 1/3:[notContains(StatusBar)]	Entry: 3/3
        // LayerTraceEntrySubject(0d0h0m0s0ms (timestamp=3))
        val mapOfFlattenedLayersAllVisibilityAssertions_fail3: Map<Int, Array<Layer>> =
            mapOf(1 to arrayOf(), 2 to arrayOf(), 3 to arrayOf())

        val expectedFailElementLifecycle4_AllVisibilityAssertions_id2 =
            LayersElementLifecycle(mutableListOf(layer_id2_t1))
        ...
        val mapOfFlattenedLayersAllVisibilityAssertions_fail4: Map<Int, Array<Layer>> =
            mapOf(1 to arrayOf(), 2 to arrayOf(layer_id2_t2), 3 to arrayOf())

        val expectedElementLifecycles_deprecated =
            mapOf(
        ...
        val expectedElementLifecyclesVisibilityAssertionProducer_deprecated =
            mapOf(
                1 to expectedElementLifecycle_id1,
                2 to expectedElementLifecycle_id2,
                3 to expectedElementLifecycle_id3,
        ...
        val expectedElementLifecyclesVisibilityAssertionProducer =
            mapOf(
                ComponentNameMatcher.NAV_BAR to expectedComponentMatcherLifecycle_Navbar,
                ComponentNameMatcher.STATUS_BAR to expectedComponentMatcherLifecycle_Statusbar,
                ComponentNameMatcher.LAUNCHER to expectedComponentMatcherLifecycle_Launcher
        ...
        val expectedElementLifecyclesAllVisibilityAssertions =
            mapOf(
                ComponentNameMatcher.STATUS_BAR to
        ...
        private fun createTraceEntries(): Array<BaseLayerTraceEntry> {
            ...
            for ((timestamp, flattenedLayers) in mapOfFlattenedLayers) {
                val layerTraceEntryBuilder =
                    LayerTraceEntryBuilder(timestamp, flattenedLayers, arrayOf(), 1)
