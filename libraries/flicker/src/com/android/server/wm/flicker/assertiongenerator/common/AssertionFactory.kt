...
class AssertionFactory(
    ...
    /**
     * Produce assertions based on the given traces and their configurations.
     * traceDumps and traceConfigurations should correspond to each other (same order)
     */
    private fun produce(
        ...
        if (traceDumps.size != traceConfigurations.size) {
            throw RuntimeException("TraceDumps and TraceConfigurations should have the same " +
        ...
        return producers.flatMap { producer ->
            producer.produce(traceContents)
        }.toTypedArray()
    }
