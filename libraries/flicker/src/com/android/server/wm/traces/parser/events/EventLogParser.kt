...
package com.android.server.wm.traces.parser.events

import com.android.server.wm.traces.common.Timestamp
import com.android.server.wm.traces.common.events.CujEvent
import com.android.server.wm.traces.common.events.CujEvent.Companion.JANK_CUJ_BEGIN_TAG
import com.android.server.wm.traces.common.events.CujEvent.Companion.JANK_CUJ_CANCEL_TAG
import com.android.server.wm.traces.common.events.CujEvent.Companion.JANK_CUJ_END_TAG
...
import com.android.server.wm.traces.common.events.FocusEvent.Companion.INPUT_FOCUS_TAG
...
class EventLogParser : AbstractParser<Array<String>, EventLog>() {
    ...
    override fun doParse(input: Array<String>): EventLog {
        val events =
            input.map { log ->
                ...
                val timestamp = Timestamp(unixNanos = rawTimestamp.replace(".", "").toLong())
    ...
    fun parse(bytes: ByteArray, from: Long, to: Long): EventLog {
        require(from < to) { "'to' needs to be greater than 'from'" }
        return doParse(
            this.doDecodeByteArray(bytes)
                .dropWhile { getTimestampFromRawEntry(it).unixNanos < from }
                .dropLastWhile { getTimestampFromRawEntry(it).unixNanos > to }
                .toTypedArray()
        )
    }

    private fun getTimestampFromRawEntry(entry: String): Timestamp {
        val (metaData, eventData) = entry.split(":", limit = 2).map { it.trim() }
        val (rawTimestamp, uid, pid, tid, priority, tag) = metaData.split("\\s+".toRegex())
