...
public class TombstoneParser {
    ...
    private static final Pattern BACKTRACE_PATTERN =
            Pattern.compile(
                    " *#(?<index>\\d+) pc (?<programcounter>\\p{XDigit}{8,16}) "
                            + " (?<filename>.+?)(?:"
    ...
    public static boolean parseTombstone(String tombstoneBlob, Tombstone.Builder tombstoneBuilder) {
        ...
        Map<Integer, com.android.server.os.TombstoneProtos.Thread> threadMap = new HashMap<>();
        try {
            // get main thread
            if (!parseMainThread(headerAndMainThreadBlob, tombstoneBuilder, threadMap)) {
            ...
            for (int i = 1; i < threadBlobs.length; i++) {
                ...
                threadMap.put(threadBuilder.getId(), threadBuilder.build());
            }
        } finally {
            tombstoneBuilder.putAllThreads(threadMap);
        }
    ...
    private static boolean parseMainThread(
            String headerAndMainThreadBlob,
        ...
        try {
            ...
            threadMap.put(mainThreadBuilder.getId(), mainThreadBuilder.build());
        }
