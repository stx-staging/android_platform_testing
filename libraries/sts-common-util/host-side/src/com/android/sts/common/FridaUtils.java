...
public class FridaUtils implements AutoCloseable {
    ...
    private static final long XZ_TIMEOUT = 10_000;
    ...
    private FridaUtils(ITestDevice device, IBuildInfo buildInfo)
        ...
        try {
            ...
            assertTrue("Frida download URL is not an XZ archive!", fridaUrl.endsWith(".xz"));
            CLog.d("Downloading Frida from %s", fridaUrl);

            try {
                // Download Frida archive
                URL url = new URL(fridaUrl);
                URLConnection conn = url.openConnection();
                File downloadedXZ = FileUtil.createTempFile("STS", fridaExeName + ".xz");
                FileUtil.writeToFile(conn.getInputStream(), downloadedXZ);

                // Extract Frida using xz utility
                CommandResult res =
                        RunUtil.getDefault()
                                .runTimedCmd(
                                        XZ_TIMEOUT,
                                        "xz",
                                        "--decompress",
                                        downloadedXZ.getAbsolutePath());
                assertEquals(
                        String.format(
                                "xz command failed with exit code %d. stdout: %s\n stderr:%s.",
                                res.getExitCode(), res.getStdout(), res.getStderr()),
                        CommandStatus.SUCCESS,
                        res.getStatus());
                String xzFilePath = downloadedXZ.getAbsolutePath();
                File extracted = new File(xzFilePath.substring(0, xzFilePath.length() - 3));

                // Copy extracted file to testCases location
                localFridaExe = new File(buildHelper.getTestsDir(), fridaExeName);
                FileUtil.copyFile(extracted, localFridaExe);
                CLog.d("extracted frida to: %s", localFridaExe);

                // Delete temp file. XZ file is already deleted by xz when extracting
                extracted.delete();
    ...
    public ByteArrayOutputStream withFridaScript(final String fridaJsScriptContent, int pid)
        ...
        try {
            ...
            CLog.e(e);
            CLog.e("Frida attach output: %s", output.toString());
            throw e;
        }
        return output;
    }
